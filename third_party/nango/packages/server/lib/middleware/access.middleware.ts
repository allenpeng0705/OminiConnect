...
export class AccessMiddleware {
    async externalTrustedAuth(req: Request, res: Response<any, RequestLocals>, next: NextFunction) {
        const mode = process.env['NANGO_AUTH_MODE']?.trim().toLowerCase();
        if (mode !== 'external_trust') {
            res.status(401).send({ error: { code: 'unauthorized', message: 'external trust auth mode is not enabled' } });
            return;
        }

        const expectedKey = process.env['OMINICONNECT_INTERNAL_KEY']?.trim();
        if (!expectedKey) {
            res.status(500).send({ error: { code: 'server_error', message: 'OMINICONNECT_INTERNAL_KEY is not configured' } });
            return;
        }

        const providedKey = req.get('x-omini-internal-key')?.trim();
        if (!providedKey || !stringTimingSafeEqual(providedKey, expectedKey)) {
            res.status(401).send({ error: { code: 'unauthorized', message: 'invalid internal trust key' } });
            return;
        }

        const rawEnvId = req.get('x-nango-environment-id')?.trim();
        const environmentId = rawEnvId ? Number.parseInt(rawEnvId, 10) : NaN;
        if (!rawEnvId || Number.isNaN(environmentId) || environmentId <= 0) {
            res.status(400).send({ error: { code: 'invalid_env', message: 'x-nango-environment-id header is required' } });
            return;
        }
