...
Provider: `granola-mcp` | Engine: `nango` | Auth: MCP OAuth2 via Nango

## Overview

These tools wrap the Granola MCP API. They allow AI agents to manage notes, collections, and collaborate with team members. Granola MCP uses Model Context Protocol for enhanced AI integration.

## Authentication

**Nango MCP OAuth2**:
- User authenticates via Nango Connect with Granola
- Uses MCP (Model Context Protocol) for AI agent integration
- Token stored in Nango, accessed via `connection_ref`
- Default scope: `offline_access`
...
## Granola MCP API Notes

- **Protocol**: MCP (Model Context Protocol) for enhanced AI integration
