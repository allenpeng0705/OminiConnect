# Tailscale (API Key) Tools

Provider: `tailscale-api-key` | Engine: `nango` | Auth: API Key via Nango

## Overview

Tailscale is a zero-config VPN that makes devices accessible from anywhere. **Requires api key via nango.**

## Authentication

**Nango API Key**:
- User provides their Tailscale (API Key) API key
