# Jamf Pro Basic Auth Tools

Provider: `jamf-basic` | Engine: `nango` | Auth: Two-step Auth via Nango
...
**Nango Two-step Auth**:
- User provides username and password via Nango Connect
- First step: Get auth token using basic auth with credentials
- Second step: Use auth token for API requests
- Token URL: https://{instance}.jamfcloud.com/api/v1/auth/token
...
## Jamf Pro Basic Auth API Notes

- **API Base URL**: https://{instance}.jamfcloud.com
- **Auth Mode**: Two-step (get token, then use token)
- **Instance**: Configured via connection config
