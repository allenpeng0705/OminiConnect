# Greenhouse Harvest OAuth2 CC Tools

Provider: `greenhouse-harvest-oauth2-cc` | Engine: `nango` | Auth: OAuth2 Client Credentials via Nango

## Overview

These tools wrap the Greenhouse Harvest API. They allow AI agents to manage candidates, jobs, applications, and offers using OAuth2 Client Credentials authentication.
...
- Token URL: https://auth.greenhouse.io/token?grant_type=client_credentials
- Auth method: basic
- Sub claim configured via connection config
...
## Greenhouse Harvest OAuth2 CC API Notes

- **Base URL**: https://harvest.greenhouse.io
- **Auth Mode**: OAuth2 Client Credentials
- **Scope Format**: `harvest:{resource}:{action}`
- **Scopes**: Configured during OAuth app registration
- **Sub Claim**: Configured via connection config for multi-tenant access
