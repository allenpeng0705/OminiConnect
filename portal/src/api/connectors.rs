...
use crate::connector_scope::{
    catalog_entry_key, find_duplicate_platform_for_entry, oauth_vault_platform_key,
};
    ...
    match connectors {
        Ok(list) => {
            ...
            let json: Vec<serde_json::Value> = list
                ...
                .map(|c| {
                    let redirect_uri =
                        if uses_nango_oauth_callback(&c.engine, &c.provider_key, &c.redirect_uri)
                            && !portal.is_empty()
                        {
    ...
    let connector = match state.connectors.get(owner, &platform).await {
        ...
        Ok(None) => {
            return (
                ...
                Json(serde_json::json!({ "error": "not configured" })),
            )
                .into_response();
        }
        Err(e) => {
