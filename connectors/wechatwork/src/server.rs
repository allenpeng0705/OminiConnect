    ...
    async fn call_tool_internal(
        ...
        match tool_name {
            ...
            "external_contact_get" => {
                let external_userid = arguments
                    .get("external_userid")
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                let body = json!({ "external_userid": external_userid });
                api.call_api("POST", "/cgi-bin/externalcontact/get", token, Some(body))
                    .await
            }
            "external_contact_tag_list" => {
                api.call_api("GET", "/cgi-bin/externalcontact/tag/list", token, None)
                    .await
            }
            "external_contact_tag_add" => {
                ...
                api.call_api(
                    "POST",
                    "/cgi-bin/externalcontact/tag/add",
                    token,
                    Some(body),
                )
                .await
            ...
            "message_send" => {
                ...
                api.call_api("POST", "/cgi-bin/message/send", token, Some(body))
                    .await
            }
            "message_list" => {
            ...
            "user_get" => {
                ...
                api.call_api("POST", "/cgi-bin/user/get", token, Some(body))
                    .await
            }
            "department_list" => {
            ...
            "user_list_by_department" => {
                let department_id = arguments
                    .get("department_id")
                    .and_then(|v| v.as_i64())
                    .unwrap_or(1);
                let fetch_child = arguments
                    .get("fetch_child")
