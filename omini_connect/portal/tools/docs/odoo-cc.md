# Odoo (Client Credentials) Tools

Provider: `odoo-cc` | Engine: `nango` | Auth: Two-Step (Client Credentials) via Nango

## Overview

These tools wrap the Odoo REST API using Client Credentials authentication. They allow AI agents to manage contacts, projects, tasks, sales orders, and CRM leads. **Requires Odoo Consumer ID and Secret for Client Credentials flow.**

## Authentication

**Two-Step (Client Credentials)**:
- Uses OAuth2 Client Credentials flow
- Consumer ID and Secret passed via Nango
- Token URL: `https://{serverUrl}/restapi/1.0/common/oauth2/access_token`
- Base URL: `https://{serverUrl}/restapi`
...
## Odoo Client Credentials Notes

- **Server URL**: Your Odoo instance domain
- **Consumer credentials**: App Consumer ID and Secret (not user credentials)
